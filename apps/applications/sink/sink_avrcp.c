...
#ifdef My_BTVol
    const uint8 avrcp_vol_convert[17]={0,8,16,24,32,40,48,56,64,72,80,88,96,104,112,127,127};
#else
...
#ifdef My_BTVol
	static uint8 sinkAvrcpConvertAvrcpVolumeToA2dpStep(uint8 avrcp_volume)   
	{    
		uint8 i;    
		for(i=0;i<17;i++)    
		{
			if(avrcp_volume<=avrcp_vol_convert[i])
				return i;    
		}    
                return 0;
	} 
	
	static uint8 sinkAvrcpConvertA2dpStepToAvrcpVolume(uint8 a2dp_step)
	{    
		return avrcp_vol_convert[a2dp_step];
	}
#else
...
static void sendAvrcpPassthroughCmd(avc_operation_id op_id, uint8 state, uint16 Index)
{
    ...
    AVRCP_DEBUG(("  *DEBUG_INFO_IOS13_whether into here\n"));
}
    ...
    else if (id == AVRCP_CONTROL_SEND)
    {
        ...
        switch (((const AVRCP_CONTROL_SEND_T *)message)->control)
        {
            ...
            case AVRCP_CTRL_PLAY_RELEASE:
                ...
                AVRCP_DEBUG(("  DEBUG_INFO_IOS13_Sending Play Released%d\n",Index));
                break;
