...
static void ButtonsUpdateButtonDebounceMask(pio_common_allbits *bMask);
...
/* function for solve button */
		PioSetDeepSleepEitherLevelBank(0, mask.mask[0], mask.mask[0]);
		PioSetWakeupStateBank(0, mask.mask[0], mask.mask[0]);
#else
...
void ButtonsCheckForChangeAfterInit(bool useButtonDebounceConfiguration)
{
    ...
    ButtonsUpdateButtonDebounceMask(&lButtonsTask->pio_debounce_mask);
...
{   
    ...
    switch ( pId )
    {
...
#if 1
            //auto power off setting 20200109 lwy
            MessageCancelAll(&theSink.task, EventUsrGaiaUser11);
            MessageSendLater(&theSink.task, EventUsrGaiaUser11, 0, D_SEC(5));
            B_DEBUG(("Main EventUsrGaiaUser11 end[%x]\n",1));
            //add end
#endif
        ...
        case MESSAGE_CHARGER_CHANGED:
        {
            ...
            PioCommonBitsRead(&pioState);
            PioCommonXORWithInvertMask(&pioState);
            flagState.flags = ((uint32)m->vreg_en_high << VREG_PIN) | ((uint32)m->charger_connected << CHG_PIN);
...
static void ButtonsUpdateButtonDebounceMask(pio_common_allbits *bMask)
{
    /*Unmask the Vreg,Charger pin mask from pio debounce*/
    if(bMask!=NULL)
        bMask->mask[0] &= ~(VREG_PIN_MASK | CHG_PIN_MASK);
}
