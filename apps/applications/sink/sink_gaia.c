...
static bool gaia_handle_control_command(Task task, GAIA_UNHANDLED_COMMAND_IND_T *command)
{
    ...
    case GAIA_COMMAND_SET_LED_CONTROL:
		//GAIA_DEBUG(("Left_Vol_Info_0207:%d \n",command->payload[0]));//add by lwy      
		if((command->size_payload > 0)) 
		{
			//GAIA_DEBUG(("G: SET_LED_CONTROL: 0\n"));	
			GAIA_DEBUG(("Left_Vol_Info_0207:%d \n",command->payload[0]));  
			My_theSink.DA7212_Vol = command->payload[0];
			gaia_send_success(GAIA_COMMAND_SET_LED_CONTROL);

			MessageSendLater(&theSink.task, EventUsrGaiaUser18, 0, 100);//add by lwy 
		}
...
/* app control auto power off add by lwy*/
    case GAIA_COMMAND_SET_BASS_BOOST_CONTROL:
        GAIA_DEBUG(("sleep_time_Info_0215:%d \n",command->payload[0]));
        if((command->payload[0] > 0)||(command->payload[0] == 0))
        {
            GAIA_DEBUG(("G: GAIA_COMMAND_SET_BASS_BOOST_CONTROL: 0\n"));
            GAIA_DEBUG(("sleep_time_Info_0215:%d \n",command->payload[0]));
            My_theSink.time_BToff = command->payload[0];
            gaia_send_success(GAIA_COMMAND_SET_BASS_BOOST_CONTROL);

            MessageSendLater(&theSink.task, EventUsrGaiaUser19, 0, 50);
        }
    ...
    case GAIA_COMMAND_GET_BASS_BOOST_CONTROL:
        ...
        // add by kimny for update sleep time infor
        GAIA_DEBUG(("sleep_time_Info_0295:%d \n",My_theSink.time_BToff));
        response[0] = My_theSink.time_BToff;
    ...
    case GAIA_COMMAND_GET_USER_EQ_CONTROL:
	    ...
	    //modify by lwy 20200108
	        ...
	        return TRUE;
		#endif
		GAIA_DEBUG(("G:GAIA_COMMAND_GET_USER_EQ_CONTROL\n"));
		response[0]=My_theSink.DA7212_EQ;
		GAIA_DEBUG(("HS:My_theSink.DA7212_EQ=0x%d\n",My_theSink.DA7212_EQ));
		gaia_send_success_payload(GAIA_COMMAND_GET_USER_EQ_CONTROL, 1, response);
		return TRUE;
	

    case GAIA_COMMAND_SET_USER_EQ_CONTROL:
        GAIA_DEBUG(("EQ_Mode_Info_0220:%d \n",command->payload[0]));//add by lwy
        if((command->size_payload ==1))
        {
            My_theSink.DA7212_EQ = command->payload[0];
            gaia_send_success(GAIA_COMMAND_SET_USER_EQ_CONTROL);
            GAIA_DEBUG(("G:GAIA_COMMAND_SET_USER_EQ_CONTROL\n"));

            MessageSendLater(&theSink.task, EventUsrGaiaUser2, 0, 100);//add by lwy 20190709
        }
...
bool gaiaGetBdAddrGaiaDevice_1(void)
{
    typed_bdaddr gaia_taddr;
