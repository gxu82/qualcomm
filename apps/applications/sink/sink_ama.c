...
/****************************************************************************/
static void getAmaDeviceInfo(ama_device_config_t *device_info)
{
    uint16 len_words = 0;
    uint16 len_bytes = 0;
    ...
    if (len_words)
    {
        ...
        len_bytes = ByteUtilsGetPackedStringLen((const uint16*)read_data->name, len_words);
        device_info->name = PanicUnlessMalloc(len_bytes + 1);
        ByteUtilsMemCpyUnpackString((uint8 *)device_info->name, (const uint16*)read_data->name, len_bytes);
        device_info->name[len_bytes] = '\0';
