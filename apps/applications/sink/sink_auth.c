...
/****************************************************************************/
static uint16 sinkAuthGetKeyDistribution(const CL_SM_IO_CAPABILITY_REQ_IND_T* ind)
{
    TRANSPORT_T transport = ind->tpaddr.transport;
    uint8 type = ind->tpaddr.taddr.type;
    bool sm_over_bredr = ind->sm_over_bredr;
    
    if(transport == TRANSPORT_BREDR_ACL)
        return KEY_DIST_NONE;
    
    /* Ensure we have IRK required to resolve random address */
    if(type == TYPED_BDADDR_RANDOM || sm_over_bredr)
        return KEY_DISTRIBUTION_RANDOM;
    
    if(sinkBroadcastAudioIsActive() && gapBaGetAssociationInProgress())
        return KEY_DIST_NONE;
    
    return KEY_DISTRIBUTION_PUBLIC;
}
...
void sinkHandleIoCapabilityInd(const CL_SM_IO_CAPABILITY_REQ_IND_T* ind)
{
    ...
    if(can_pair)
    {
        uint16 key_dist = sinkAuthGetKeyDistribution(ind);
#ifdef ENABLE_FAST_PAIR
        ...
        if(sinkBroadcastAudioIsActive() && gapBaGetAssociationInProgress())
        {
            if((key_dist == KEY_DISTRIBUTION_RANDOM) || (ind->tpaddr.transport == TRANSPORT_BREDR_ACL ))
            {
                bonding = TRUE;
            }
            else
        ...
        ConnectionSmIoCapabilityResponse(&ind->tpaddr, local_io_capability, sink_mitm_setting, bonding, key_dist, oob_data_none, NULL, NULL);
    }
    else
    {
