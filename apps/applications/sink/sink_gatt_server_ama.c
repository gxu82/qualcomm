...
void SinkAmaServerUpdateConnectioParameters(uint16 cid, bool fast);
...
void SinkAmaServerUpdateConnectioParameters(uint16 cid, bool fast)
{
	...
	{
		ConnectionDmBleConnectionParametersUpdateReq( sink_gatt_ama_server_task->app_task, &tpaddrt->taddr, CONNECTION_PARAMTER_INTERVAL_MIN, CONNECTION_PARAMTER_INTERVAL_MAX, CONNECTION_PARAMTER_UPDATE_LATENCY, 200, 0 , 0);
	}
	free(tpaddrt);
}
...
static void registerDmAmReq(uint16 handle)
{
	 DM_AM_REGISTER_REQ_T *prim;



	 prim = PanicUnlessNew(DM_AM_REGISTER_REQ_T);

	 prim->type = DM_AM_REGISTER_REQ;
	 prim->phandle = handle;
	 VmSendDmPrim( prim );
}
...
static void SendDisableSlaveLatency(const typed_bdaddr* taddr, uint16 handle, bool enabled)
{


	 DM_ULP_ENABLE_ZERO_SLAVE_LATENCY_REQ_T *prim = PanicUnlessNew(DM_ULP_ENABLE_ZERO_SLAVE_LATENCY_REQ_T);



	 /* Note: the line registerDmAmReq(handle); can be removed from SendDisableSlaveLatency function and should be executed only once. */

	 registerDmAmReq(handle);

	 prim->type = DM_ULP_ENABLE_ZERO_SLAVE_LATENCY_REQ;
	 prim->phandle = handle;
	 prim->zero_latency = enabled;


	 prim->tp_addrt.tp_type = LE_ACL;
	 prim->tp_addrt.addrt.type = taddr->type;

	 BdaddrConvertVmToBluestack( &prim->tp_addrt.addrt.addr, &taddr->addr );
...
/*******************************************************************************
NAME
    sinkGattGetAmaServerUpdateRundata
...
*/
#if 0
static bool sinkGattGetAmaServerUpdateRundata(uint16 **ptr)
{
    bool status = FALSE;

    gattServerSetServicePtr(ptr, gatt_server_ama);
    *ptr += sizeof(GBLETEST_T);
    status = TRUE;
...
bool sinkGattAmaServerInitialise(uint16 **ptr)
{
    ...
    if (!sinkGattAmaServerInitTask(ptr))
    {
        return FALSE;
    }
...
extern uint16 VmSetUlpRepeat (uint16 tout);


uint16 PsStore(uint16 key, const void *buff, uint16 words);

uint16 VmSetUlpRepeat (uint16 tout)
{
    uint16 result;

    result = PsStore(0x8010, (const void *)tout, 0);
...
void sinkGattAmaServerMsgHandler(Task task, MessageId id, Message message)
{
	...
	/* GATT_AMA_SERVER_INFO(("GATT MsgHan\n")); */
	// GATT_AMA_SERVER_INFO((" AMA server message id   \n",id ));
	...
	if(id == GATT_AMA_SERVER_WRITE_IND)
	{
        ...
        /* sinkAmaServerNotify( sink_gatt_ama_server_task->cid, HANDLE_AMA_ALEXA_RX_CHAR, 2, NULL);*/
	}
