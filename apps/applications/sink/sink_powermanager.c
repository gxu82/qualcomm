    ...
    if(!LedManagerChargerTermLedOveride()) 
    {
        ...
        switch(state)
        {
            ...
            case power_charger_boost_external:               
                 //PsStore(PSKEY__USR99, &My_theSink.DA7212_Vol, 3);//add by lwy modify by lwy20200904 xv3_sf
                 MessageSendLater(&theSink.task, EventUsrGaiaUser6, 0, D_SEC(2));//add by lwy 20190709
