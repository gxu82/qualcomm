...
//bool Prompt_Done_flag=FALSE;

uint8 key_cnt=0; //for pio0 press check


/* add by lwy */
My_hsTaskData My_theSink;
    ...
    else
    {
        switch(EventPower)
        {
            ...
            case EventSysPowerOnPanic:
                ...
                PsRetrieve(PSKEY__USR99, &My_theSink.DA7212_EQ, 3);
                DEBUG(("HS: My_theSink.DA7212_EQ= 0x%x \n",My_theSink.DA7212_EQ));
                DEBUG(("HS: My_theSink.DA7212_Vol= 0x%x \n",My_theSink.DA7212_Vol));                
                DEBUG(("HS: My_theSink.time_BToff= 0x%x \n",My_theSink.time_BToff));

                MessageSendLater(&theSink.task, EventUsrGaiaUser11, 0, D_SEC(5));//auto power off
         
                PioSetPio(AMP_EN,1,1);
                //MAIN_DEBUG(("HS: AMP enable\n" )) ;
                DA7212_Init();
            ...
            case EventUsrPowerOff:	
                        ...
                        sinkDataAllowAutomaticPowerOffWhenCharging()) && powerManagerIsVbatCritical()) || powerManagerIsVthmCritical())
                {
                    ...
                    PsStore(PSKEY__USR99, &My_theSink.DA7212_EQ, 3);
                    DEBUG((" HS: My_theSink.DA7212_Vol= 0x%x \n",My_theSink.DA7212_Vol));
                    DEBUG((" HS: My_theSink.DA7212_EQ= 0x%x \n",My_theSink.DA7212_EQ));

                    //Prompt_Done_flag=1;

                    MessageSendLater(&theSink.task, EventUsrGaiaUser6, 0, 2500);
                }
                else
                {
                    indicate_event = FALSE ;
...
static void handleUEMessage  ( Task task, MessageId id, Message message )
{
    ...
    switch ( id )
    {
        case EventUsrGaiaUser1:
            DA7212_EQ_Mode_Up();//button control eq mode 
            MessageSendLater(&theSink.task, EventUsrGaiaUser7, 0, 0);//add by lwy 20200904 保存EQ值
            MAIN_DEBUG(("HS: EventUsrGaiaUser1\n" )) ;
            break;

        case EventUsrGaiaUser2:       
            DA7212_UpdateEQ();//contorled by app 
            MessageSendLater(&theSink.task, EventUsrGaiaUser7, 0, 0);//add by lwy 20200904 保存EQ值
            MAIN_DEBUG(("HS: EventUsrGaiaUser2\n" )) ;
            break;

        case EventUsrGaiaUser3:
        	Volume_Up();

			payload[0]=My_theSink.DA7212_Vol;
            //MAIN_DEBUG(("HS: My_theSink.DA7212_Vol=0x%d\n",My_theSink.DA7212_Vol )) ;
            //if(gaiaGetBdAddrNonGaiaDevice(FALSE))
            if(gaiaGetBdAddrGaiaDevice_1())
            {
                gaia_send_response(GAIA_VENDOR_QTIL, GAIA_COMMAND_GET_LED_CONTROL, GAIA_STATUS_SUCCESS, 1, payload);
            }
			MessageSendLater(&theSink.task, EventUsrGaiaUser7, 0, 0);//add by lwy 20200904
        	break;

        case EventUsrGaiaUser4:
			Volume_Down();	
        ...
        case EventUsrGaiaUser5:
        	DA7212_PlayEQVoicePrompt_Delay();
            break;

        case EventUsrGaiaUser6:
            DA7212_Mute_HP();
            //MAIN_DEBUG(("HS: EventUsrGaiaUser6\n" )) ;
        break;

        case EventUsrGaiaUser7:
			PsStore(PSKEY__USR99, &My_theSink.DA7212_EQ, 3);//20200904 add by lwy xv3_sf
            break;

        case EventUsrGaiaUser8:
        	break;

        case EventUsrGaiaUser9:
        	break;

        case EventUsrGaiaUser10:
        	break;

        case EventUsrGaiaUser11:
			AVRCP_Update_Sleeptime_NoneVoiceTone(); 
			MessageSendLater(&theSink.task, EventUsrGaiaUser7, 0, 0);//add by lwy 20200904
        break;
  
        case EventUsrGaiaUser12:
            DA7212_Mute();//mute input channel
            MAIN_DEBUG(("HS: EventUsrGaiaUser12\n" )) ;
            break;

       case EventUsrGaiaUser13:
            payload[0]=My_theSink.DA7212_EQ;
            //MAIN_DEBUG(("HS: EventUsrGaiaUser13\n" )) ;
            //MAIN_DEBUG(("HS: My_theSink.DA7212_EQ=0x%d\n",My_theSink.DA7212_EQ )) ;
            //MAIN_DEBUG(("HS: %d\n",payload[0] )) ;
            if(gaiaGetBdAddrGaiaDevice_1())
            {
                gaia_send_response(GAIA_VENDOR_QTIL, GAIA_COMMAND_GET_USER_EQ_CONTROL, GAIA_STATUS_SUCCESS, 1, payload);
            }
        break;
               
       case EventUsrGaiaUser14:
        	break;

       case EventUsrGaiaUser15:
        	break; 
        	                                              
       case EventUsrGaiaUser17:
            MessageSendLater(&theSink.task, EventUsrPowerOff, 0, D_MIN(1));//voice tone 'one minute later auto power off'
            MAIN_DEBUG(("HS: EventUsrGaiaUser17\n" )) ;
        	break;

        case EventUsrGaiaUser18:
            App_control_DA7212_Vol();
            MAIN_DEBUG(("HS: EventUsrGaiaUser18\n" )) ;
			MessageSendLater(&theSink.task, EventUsrGaiaUser7, 0, 0);//add by lwy 20200904
        	break;

        case EventUsrGaiaUser19:
            AVRCP_Update_Sleeptime();
            MAIN_DEBUG(("HS: EventUsrGaiaUser19\n" )) ;
			MessageSendLater(&theSink.task, EventUsrGaiaUser7, 0, 0);//add by lwy 20200904
        	break;

        case EventUsrGaiaUser20:        		
        	break;
        ...
        case (EventUsrEnterPairing):
            ...
            #if 1
				if(id == EventSysEnterPairingEmptyPDL ) 
					{
						AudioPromptPlayEvent(EventUsrEnterPairing,1);//add by lwy 20191219
					}
...
/* add by lwy */
                ...
                My_theSink.DA7212_Vol = 9;
                My_theSink.time_BToff = 1;
				DEBUG((">> Reset_HS: My_theSink.DA7212_EQ= 0x%x \n",My_theSink.DA7212_EQ));
				DEBUG((">> Reset_HS: My_theSink.DA7212_Vol= 0x%x \n",My_theSink.DA7212_Vol));
				DEBUG((">> Reset_HS: My_theSink.time_BToff= 0x%x \n",My_theSink.time_BToff));

				//PsStore(PSKEY__USR99, &My_theSink.DA7212_EQ, 3);
                deviceManagerRemoveAllDevices();
              
                MessageSend(&theSink.task, EventUsrEnterPairing, 0);//add by lwy
        ...
        case EventSysSLCConnectedAfterPowerOn:
            ...
            lResetAutoSwitchOff = TRUE;
            	...
            	MAIN_DEBUG(("HS: while power on sys things\n")) ;//add by lwy
				MessageSendLater(&theSink.task, EventUsrBleStartBonding, 0, D_SEC(1));//add by lwy
       ...
       case EventUsrButtonLockingToggle:
        ...
        case EventSysA2dpDisconnected:
            ...
            MessageCancelAll(&theSink.task , EventUsrEnterPairing); /* add by lwy  20191028*/
        break;
      ...
      case EventSysCodecAptxAdaptiveInUse:
        ...
        case EventUsrVAPushToTalk:
            SinkVaTriggerPushToTalk();
            break;

        case EventUsrVAPushToTalkRelease:
            SinkVaTriggerPushToTalkRelease();
            break;
...
static void IndicateEvent(MessageId id)
{
    ...
    {
        //DA7212_CloseMute_HP();
        Remember_Vol0_Status();
    }

    sinkAudioIndicationPlayEvent(id);

	//Prompt_Done_flag=FALSE;
