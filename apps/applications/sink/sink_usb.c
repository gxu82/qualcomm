...
#endif /*ENABLE_USB_AUDIO_APP_STEREO_DESCRIPTORS*/    
    {SAMPLE_RATE_CVC, &usb_cvc_config, &usb_cvc_config, &usb_cvc_config, usb_plugin_mono_nb},
    {SAMPLE_RATE_CVC_WB, &usb_cvc_wb_config, &usb_cvc_wb_config, &usb_cvc_wb_config, usb_plugin_mono_wb}
};
...
static const usb_plugin_info* usbAudioGetPluginInfo(Task* task, usb_plugin_type type, uint8 index)
{
    switch(type)
    {
        ...
        case usb_plugin_mono_nb:
            *task = AudioPluginVoiceVariantsGetUsbPlugin(usb_voice_mono_nb, index);

            audioControlLowPowerCodecs (TRUE) ;
        break;
        case usb_plugin_mono_wb:
            *task = AudioPluginVoiceVariantsGetUsbPlugin(usb_voice_mono_wb, index);
...
static void usbAudioSetVolume(void)
{
    if(usbAudioSinkMatch(sinkAudioGetRoutedAudioSink()) && usbMicOrSpeakerEnabled())
    {
        AUDIO_MODE_T mode;
...
{
    if(usbAudioSinkMatch(sinkAudioGetRoutedAudioSink()) && (sinkUsbconfigGetPluginType() == usb_plugin_stereo))
    {
