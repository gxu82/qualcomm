...
void sinkHfpInit( void )
{
    ...
    if(!AudioPluginVoiceVariantsGetHfpPlugin(hfp_wbs_codec_mask_msbc, sinkHfpDataGetAudioPlugin()) &&
...
void sinkAppInit( const HFP_INIT_CFM_T *cfm )
{
    ...
    {
        /* Initiailize Broadcast Audio module only if we are already in any of BA modes */
...
void InitEarlyUserFeatures ( void ) 
{
    ...
    {
        stateManagerEnterLimboState(limboShutDown);// add by lwy
    }
}
