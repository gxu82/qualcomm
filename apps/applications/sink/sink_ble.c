...
static const ble_configuration_t ble_config = {
                                ...
                                30,     /* Bondable Pairing timeout (secs) */
#else
                                ...
                                0,//30,     /* Fast adv timer */ //modify by lwy 20200224
#ifdef GATT_AMA_SERVER
...
#ifdef ENABLE_FAST_PAIR                                
                                32,   /* Fast Pair adv interval min */
                                48,   /* Fast Pair adv interval max */
                                15   /* Fast Pair adv timer (secs) */
#endif                                
};
...
#ifdef ENABLE_FAST_PAIR
    /*This is needed before switch between Public and Random Address*/
    sinkGattManagerStopAdvertising();

    if (bleGetFPadvertMode() == fp_advertising_reconnection)
    {
        /*For fast pair reconnection case set it to random addr - else should fall back to public */
        random_addr = TRUE;
    }
#endif
...
#ifdef ENABLE_FAST_PAIR
        adv_params->interval_min = sinkBleGetConfiguration()->fast_pair_adv_interval_min;
        adv_params->interval_max = sinkBleGetConfiguration()->fast_pair_adv_interval_max;
#endif
    }
    else
    {
