...
DESCRIPTION
    Returns the A2DP Suspend State for the specified index.

RETURNS
    a2dp suspend state

**************************************************************************/
static a2dp_suspend_state getA2dpSuspendState(uint16 index)
{
...
DESCRIPTION
    Updates the A2DP Suspend State for the specified index.

RETURNS
    None

**************************************************************************/
void setA2dpSuspendState(uint16 index, a2dp_suspend_state state)
{
...
void a2dpSetSuspendState (uint16 id, a2dp_suspend_state state)
{
    ...
    {   /* Only update the suspend state of the device only if it was not set to remote or locally suspended  previously */
        if (getA2dpSuspendState(id) == a2dp_not_suspended)
        {
            setA2dpSuspendState(id, state);
        }
    }
    A2DP_DEBUG(("A2dp: SuspendState[%u] = %d\n",id, getA2dpSuspendState(id)));
}
...
#endif
                    if ((getA2dpSuspendState(index) == a2dp_local_suspended) ||
                        (sinkA2dpOpenMediaOnConnection()))
                    {
...
void handleA2DPOpenCfm(uint8 DeviceId, uint8 StreamId, uint8 seid, a2dp_status_code status)
{
    ...
    if (status == a2dp_success)
    {
        ...
        if(A2dpDeviceGetBdaddr(DeviceId, &bd_addr) && getA2dpIndexFromBdaddr(&bd_addr , &Id))
        {
           ...
           if ((getA2dpSuspendState(Id) == a2dp_local_suspended) && (getA2dpPeerRemoteDevice(Id) != remote_device_peer))
           {
#ifdef ENABLE_AVRCP
        ...
        if(getA2dpIndex(DeviceId, &Id))
        {
            ...
            if(stateManagerGetState() == deviceA2DPStreaming)
            {
                ...
                if(sinkPartymodeIsEnabled() && sinkPartymodeGetOperatingMode())
                {
                    ...
                    state here*/
                    if(a2dp_local_suspended != getA2dpSuspendState(Id))
                    {
...
DESCRIPTION
    Used to suspend all a2dp media streams including peer 
...
void sinkA2dpResumePeerSession(void)
{
    ...
    for(index = 0; index < MAX_A2DP_CONNECTIONS; index++)
    {
        if(getA2dpPeerRemoteDevice(index) == remote_device_peer)
        {
            if(getA2dpSuspendState(index) == a2dp_not_suspended)
            {
            ...
            else if(getA2dpSuspendState(index) == a2dp_local_suspended)
            {
                a2dpIssuePeerStartRequest();
            }
        }
...
void  a2dpSuspendNonRoutedStream(a2dp_index_t index)
{
        ...
        && !getA2dpStatusFlag(PLAYING, index))
    {
        ...
        /* set the local suspend status indicator */
        a2dpSetSuspendState(index, a2dp_remote_suspended);
...
{
    return (SinkA2dpIsInitialised() ? getA2dpSuspendState(index) : a2dp_not_suspended);
}
