...
/**
 *  \brief Get the current sink write position (in bytes) of the image section sink
 *  stream, skipping the first 4 bytes.
 *   
 *  \param sink Sink handle for the image upgrade stream. 
 *  \param offset Address to return the current sink write position. 
 *  \return Successful in getting the offset for the input sink. 
 * 
 * \note This trap may NOT be called from a high-priority task handler
 * 
 * \ingroup trapset_imageupgrade
