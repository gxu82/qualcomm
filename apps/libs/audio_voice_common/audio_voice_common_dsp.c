...
static operator_filters_t* getNewFilters(operator_filters_t* filter, bool wideband)
{
    ...
    operator_config_t* ra_filter = AudioVoiceCommonGetRAFilter(wideband);
...
/******************************************************************************/
static uint16 GetLinkEncodingTypeSampleRate(link_encoding_t encoder)
{
    return (encoder == link_encoding_msbc) ? WB_SAMPLE_RATE : NB_SAMPLE_RATE;
}
/*******************************************************************************
DESCRIPTION
...
static void createOperators(audio_voice_context_t* ctx, operator_filters_t* filter)
{
    ...
    {
        operator_filters_t* new_filter = getNewFilters(filter, AudioVoiceCommonIsWideband(ctx->encoder));
