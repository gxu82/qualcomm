...
{
    uint8 *array_data = PanicUnlessMalloc(array_size * sizeof(uint16));
    ...
    if(data_array)
    {
        memmove(array_data, data_array, array_size * sizeof(uint16));
    }
    else
    {
        message->status = cl_crypto_error;
    ...
    if(valid_params)
    {
        sha256(array_data, (uint8)(array_size * sizeof(uint16)), hash);
