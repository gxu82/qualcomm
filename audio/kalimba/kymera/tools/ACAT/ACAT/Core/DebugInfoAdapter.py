...
class DebugInfoAdapter(di.DebugInfoInterface):
    ...
    def __init__(self, ker):
        ...
        info. The rest is just Debug info mapped based on the elf id in integer.
    ...
    def reload(self):
        """
        Reload Debug Info of elf files if they have changed
        """
    ...
    def get_ker_debug_infos(self):
        """
        Get all the KerDebugInfo instances.
        """
        return [
            ker_debug_info
    ...
    def _call_debuginfo_pm_addr(self, address, function_name):
        """
        @brief Decides if the address is part of a downlaodable bundle or not
            and calls _call_debuginfo_elfid with the elf id for further processing.
            This is necessary because _call_debuginfo_elfid assumes that the
            address is in Kymera or patch elf.
        ...
        if Arch.get_pm_region(address, False) is None:
            raise ct.InvalidPmAddress("Key " + hex(address) + " is not in PM")
    ...
    def _call_debuginfo_function(self, elf_id, function_name, *argv, **kwargs):
        """
        @brief Searches for the right debuginfo based on elf id (Kymera, patch or
        ...
        if (function_name in self.dm_address_mappable_function or
            function_name in self.pm_address_mappable_function) and \
            isinstance(argv[0], numbers.Integral):
            address = argv[0]
            if self._address_in_table(address):
        ...
        try:
            ...
            raise ct.BundleMissingError(hex(elf_id))
