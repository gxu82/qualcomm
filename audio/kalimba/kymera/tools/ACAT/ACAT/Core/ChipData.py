...
class ChipData(object):
    """
    @brief This class defines an interface for extracting information from
    a chip (whether it is connected 'live' or its state has been
    extracted into a coredump).
    """
    ...
    def get_patch_id(self):
        ...
        @return     An integer, zero if there is no patch detected
        """
    ...
    def set_debuginfo(self, debuginfo):
        """
        @brief Chipdata uses the debug information for getting type
    ...
    def get_var_strict(self, identifier, elf_id=None):
        """
        @brief Provided for the convenience of all Analysis modules, this method
    ...
    def get_reg_strict(self, identifier):
        ...
        Returns a DataSym containing the register details plus its value.
        ...
        if isinstance(identifier, numbers.Integral):
            ...
            # Must be a processor register? On Amber these are listed in constants, so
            # we could look up the address in the same way as for memory-mapped registers
            # below. But on other chips (e.g. Gordon) they're not (in fact, they may not
            # even be mapped into DM at all), so we need to get them from
            # chipdata by name.
            ...
            # of the register.
            # get_constant_strict might throw an exception here; make no effort to
            # intercept it because we are being strict!
    ...
    def get_banked_reg_strict(self, address, bank=None):
        """
        @brief This method looks up the values of a banked register.
        It takes the address of a banked register and the bank number (optional).

        Returns a ct.DataSym containing the bank select register name and address
        plus the values in the bank.
        @param[in] self Pointer to the current object
        @param[in] address Address of the banked register
        @param[in] bank = None
        """
    ...
    def cast(self, addr, type_def, deref_type=False, section="DM", elf_id=None):
        """
        ...
        if is_patched and isinstance(type_def, numbers.Integral) and elf_id is None: 
    ...
    def run_poll(self, time, identifier):
        """
        @brief Measures the value of a defined variable(s) over a given period of time
        @param[in] self Pointer to the current object
        @oaram[in] time
        @param[in] identifier - Name/address. Argument can be a single entry
        or in the form of a list e.g. [0x8688, '$_total_pool_size']
        """
        queue = Queue()

        _stop_event = threading.Event()
        # Detects if identifier is a list or a single entry so it can be formatted correctly.
        # If there are multiple entries there will be fewer measurements taken
        # for each variable.
...
class VariablePoller(threading.Thread):
    """
    @brief This class is responsible for reading a defined variable/register for
    a period of time. If a list of variable/register names is passed to the class
    then each shall be measured.
    """
    ...
    def run(self):
        """
        @brief Reads the desired variable until the stop event arrives, returns list.
        @param[in] self Pointer to the current object
        """
        multi_flag = 0
        poll_data = []
