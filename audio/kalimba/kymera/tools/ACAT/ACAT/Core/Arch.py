...
# perform the lookup.
# Like CoreUtils and CoreTypes, all the variables and functions are globally-scoped.
# It's easier than putting them all into a singleton class or passing an object around
# between all the analyses.
...
# There's *probably* enough information in the elf 'section hdrs' to distinguish
# the different PM and DM regions, so we should really use that. There are two
...
def chip_select(l_kal_arch, l_chip_arch, l_chip_id, l_chip_revision=None):
    ...
    * l_chip_id is an integer representing the chip id, e.g. 28
    ...
    # At this stage, the chip_id isn't necessary to distinguish between the chips
    # we support, but eventually it probably will be.
    global dRegions, pRegions, kal_arch, \
        chip_arch, chip_id, chip_cpu_speed_mhz, \
        addr_per_word, chip_revision, dm_ram_aliased
    ...
    if kal_arch == 3 and chip_arch == "Bluecore":
    ...
    elif (kal_arch == 4 and chip_arch == "Hydra" and chip_id == 0x46 and
          chip_revision <= 0x1F):
        ...
        # Stre
        dRegions = dRegions_Stre
        pRegions = pRegions_Stre
        chip_cpu_speed_mhz = 120
        addr_per_word = 4
        dm_ram_aliased = True
    elif kal_arch == 4 and chip_arch == "Hydra" and chip_id == 0x4A:
        # Stretto uses the same memory map as Stre.
        dRegions = dRegions_Stre
        pRegions = pRegions_Stre
        chip_cpu_speed_mhz = 120
        addr_per_word = 4
