...
class ProcessorPatcher(object):
    ...
    @param  patch_path  Path to a patch
    """
    ...
    def apply(self):
        """
        Check the conditions and patch the processor
    ...
    def _find_build_patch(self):
        """
        Use an external library, if available, to find the appropriate patch
        ...
        @raises ImportError     When the external library is invalid
        @raises Exception       When the provided patch_id is invalid
        """
    ...
    def _patch(self):
        ...
        if patch_path is not None:
            if not patch_path.endswith(('.elf', 'kymera-patch')):
    ...
    def _get_debug_info(build_path):
        """
        Instantiate KerDebugInfo object with the given build path

        @param  build_path  A valid path to a build
        @return An instance of KerDebugInfo object
        """
