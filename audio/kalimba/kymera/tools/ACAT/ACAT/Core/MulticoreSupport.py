...
class Functions(object):
    ...
    def _verbose(self, obj, indent=0):
        """
        @brief Converts the given object to a printable format and prints the output

        If the given object is not supported by the method, RuntimeError
        exception will be raised.

        @param[in] obj Can be any Python standard type
        """
        if isinstance(obj, dict):
            ...
            raise RuntimeError("Unsupported type for {}({})".format(str(obj), type(obj)))
    ...
    def search(self, phrase, ignore_case=True, elf_id=None, verbose=True):
        ...
        @param[in] phrase: A searching phrase in string. The phrase can
                           contain * as wildcard.
        @param[in] ignore_case A flag to indicate whether the search
                               should be case sensitive or not
        @param[in] verbose Instead of returning an object prints the results
        @param[out] A dictionary or None when in verbose mode
        """
