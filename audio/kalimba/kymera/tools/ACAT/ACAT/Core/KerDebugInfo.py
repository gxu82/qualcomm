...
class KerDebugInfo(di.DebugInfoInterface):
    ...
    def __init__(self, build_output_path):
        ...
        if build_output_path.endswith(".elf"):
            self.debuginfo_path = build_output_path.rstrip(".elf")
        else:
            self.debuginfo_path = build_output_path
        # Locate build output
        self.lst_path = os.path.normpath(self.debuginfo_path + '.lst')
        self.elf_path = os.path.normpath(self.debuginfo_path + '.elf')
    ...
    def has_changed(self):
        ...
        """
        if os.path.getmtime(self.elf_path) != self._elf_modified_time:
            return True
    ...
    def _read_debuginfo(self):
        ...
        with cu.global_options.ker.scoped_ker(self.elf_path, ["debug_strings"]) \
            ...
            try:
                logger.debug('ELF reader version: %s', kal_elf_reader.get_version())
