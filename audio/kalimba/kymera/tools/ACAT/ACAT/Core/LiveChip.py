...
def re_entrant_lock_decorator(function):
    """
    @brief Decorator function that only blocks if the lock is held by another thread.
    @param[in] function Function to decorate
    """
...
class LiveChip(ChipData.ChipData):
    ...
    def is_volatile(self):
        """
        @brief This is a live device over SPI so the contents is volatile
        @param[in] self Pointer to the current object
        """
        return True
    ...
    def get_data(self, address, length=0, ignore_overflow=False):
        ...
        try:
            if ('DM' in mem_region) or ('SLT' in mem_region) or ('MMR' in mem_region) or (
                    'NVMEM' in mem_region) or ('PMRAM' in mem_region) or ('MCU' in mem_region):
                if length == 0:
    ...
    def _get_slt_entry(self, entry):
        """
        @brief Navigates the slt looking for an entry value
        @param[in] self Pointer to the current object
        @param[in] entry
        """
