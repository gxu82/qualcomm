...
class MultilineFormatter(argparse.HelpFormatter):
    """
    An Argument Parser formatter that supports multi line help and epilog
    """
    multiline_tag = "multiline|"

    def _split_lines(self, text, width):
        """
        Override the original `_split_lines` method
    ...
    def add_text(self, text):
        """
        Override the original add_text to handle the multiline tag
...
class AcatArgumentParser(object):
    ...
    def namespace(self):
        """
        Read only property to access the namespace of parsed arguments
        """
    ...
    def populate_global_options(self, global_options):
        """
        Populate the given GlobalOptions instance

        @param global_options   A mutable GlobalOptions instance
        """
        self._parse()
        self._check()

        # Set the coredump path, if given, and increase the cache validity
        # interval.
        coredump_path = os.path.normcase(self._ns.coredump)
        if coredump_path:
            global_options.coredump_path = coredump_path
    ...
    def _parse(self):
        """
        Parse the given arguments and do the sanity check on them
        """
        ...
        self._parser.add_argument(
            '-c',
            '--coredump',
            help="Path to coredump file to analyse",
            dest='coredump',
            default='',
            type=str,
        )
        ...
        self._parser.add_argument(
            ...
            '--verbose',
            help=(
                "When set, extra logging information will appear on the screen "
                "and log files will be created in ACAT's package directory."
            ),
            dest='verbose',
            action='store_true'
        )
    ...
    def _check(self):
        """
        Do the post parse check, main focus on dependencies
        """
        is_interactive = self._ns.interactive or self._ns.ipython
    ...
    def _extract_kalcmd2(self):
        """
        Remove the kalcmd2 instance if available and assign it to an instance
        variable.

        kalcmd2 is not a command line argument, however external apis, like
        kats pass the instance by calling the parser.
        """
        if self._arguments:
