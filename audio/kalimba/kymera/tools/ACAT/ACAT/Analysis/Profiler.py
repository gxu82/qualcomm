...
def output_percentage(output_function, occurrence, total_measurements=100,
    ...
    @param[in] sort = True
    @param[in] column Which terminal column to left-justify the formatted
                      percentage to
    """
...
class Profiler(Analysis.Analysis):
    ...
    def run_all(self, run_pc_clks=False):
        """
        @brief Perform all useful analysis and send the output to the
    ...
    def run_clks(self, time):
        """
        @brief Calculates the total wake time / MCPS using NUM_RUN_CLKS.
    ...
    def ret_run_clks(self, time):
        """
        @brief Returns the total wake time / MCPS using NUM_RUN_CLKS.
        Please see RunClksReader for more details on the method.
        @param[in] self Pointer to the current object
        @param[in] time
        @return MCPS, percentage (derived), assumed CPU speed (MHz)
        """
    ...
    def run_kymera_builtin(self, detailed_debug=False, raw_debug=False):
        """
        @brief Reads the built-in Kymera profiler (if compiled in) and
        displays the interpreted measurements. This is an on-chip task which
        ...
        @param[in] self Pointer to the current object
        @param[in] detailed_debug=False Whether to also report detailed
          statistics for each profiler entry. Default is just a summary
          of MCPS. The detailed statistics, if available, include figures
          per invocation of the profiler (e.g., per-kick), such as INSTRS
          (from which MIPS can be derived). These performance counters
          are described in the Qualcomm Kalimba Architecture DSP User Guide.
        @param[in] raw_debug=False Whether to also print the raw,
          uninterpreted profiler structure from the chip. Only effective with
          detailed_debug True.
        """
