...
class StackInfo(Analysis.Analysis):
    ...
    def stack_overflow_occurred(self):
        """
        Check whether stack overflow happened
    ...
    def _get_all_stack_data(self):
        ...
        # analysis.
        if (
                start_of_stack <= stack_pointer <= end_of_stack and
    ...
    def get_stack_frames(self):
        ...
        Returns a list of tuples, each containing a stack frame. The
        oldest frame is at the start, and frames are listed with lowest-
        address first - that is, the same way DM normally appears.  Might
        raise an AnalysisError exception.
    ...
    def is_stack_address(self, address):
        """
        Check whether the given address is a location on the stack
