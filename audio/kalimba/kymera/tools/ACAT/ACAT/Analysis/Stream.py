...
class Endpoint(object):
    ...
    def compact_name(self):
        """
        Returns a tidy and short string representation of an endpoint
        object
...
class Stream(Analysis.Analysis):
    ...
    def convert_transform_id(id_to_convert, force_direction=None):
        """
        Takes a transform ID, and returns the internal/external version of it.
    ...
    def run_all(self, cbopsanalysis_on=False):
        """
        Runs a complete analysis of Streams (transforms, endpoints,
        chains, ratematch pairs, kick objects).

        If cbopsanalysis_on == True, the CBOPs mgr will also be analysed
        and displayed.
    ...
    def analyse_transforms(self, display_buffer=False, analyse_metadata=False):
        ...
        @param[in] display_buffer If True the transform buffer is displayed.
                   False by default.
        @param[in] analyse_metadata If True the metadata of the transform
    ...
    def plot_transforms_buffers(self, screen_width=80, live_plotting=True):
        ...
        @param[in] live_plotting The live_plotting mode will continuously
                                 display the buffer usage until an exit
                                 event is received.
        """
        self._read_all_transforms()
        buffers = []
    ...
    def analyse_endpoints(self, cbopsanalysis_on=False):
        ...
        If cbopsanalysis_on == True, the CBOPs mgr analysis will also be
        displayed.
    ...
    def _create_streams_html(self, operators, graph):
        ...
        @param[in] graph Graphviz object.
        """
        ...
        # Display the graph
        formtter_temp.section_start("Sreams")
...
def _path_to_url(path):
    """
    Convert a local path to a browser friendly one

    Args:
        path (str): An OS path

    Returns:
        A string that is the address of a file or location on the drive
