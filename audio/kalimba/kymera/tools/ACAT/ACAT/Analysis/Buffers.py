...
class Cbuffer(ct.Variable):
    ...
    def __init__(self, var, is_old_version):
        """
        Pseudo-copy-constructor: takes a cbuffer Variable that has been casted
        from a pointer.
    ...
    def get_previous_address(self, address):
        """
        Returns the address which is before the given address in the buffer

        This will count for buffer wrap.
...
class Buffers(Analysis.Analysis):
    ...
    def analyse_transform_buffers(self, analyse_metadata=False,
        ...
        @param[in] plot_contents whether display the buffer's contest in
                   an image or not.
        """
        try:
    ...
    def analyse_cbuffer(self, buffer_p, title_str="None",
        ...
        @param[in] title_str description of the transform to be displayed as
                   title for plot.
        @param[in] analyse_metadata If True the metadata of the buffer
                   is displayed. False by default.
        @param[in] plot_content If True generates a diagram and display it
        """
        ...
        if plot_content and plt is not None:
    ...
    def draw_cbuffer(self, buffer_p, rd_side='right'):
        """
        @brief Takes a cbuffer pointer and draw the diagram using the
        instance's formatter
    ...
    def _analyse_buffer_metadata(self, buffer_var):
        """
        It takes a Cbuffer object and returns the members of the linked list
        and the number of tags associated with them

        If tag names are passed as a list, these tags are displayed e.g[
        'index', 'flags', 'data'] If empty list [] is passed  , all tags are
        displayed
        ...
        if buff_metadata_flag:
            return 'Descriptor flag is set for metadata, but the ' \
                   'pointer to it is NULL.'
...
class PrintBufferUsage(threading.Thread):
    ...
    def get_buffer_usage_table(self):
        ...
        @param[in] self Pointer to the current object
        @param[out] String containing the formatted buffer usage table plus
                    some information describing the table.
        """
        if not self.buffers:
