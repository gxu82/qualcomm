...
class IPC(Analysis.Analysis):
    ...
    def analyse_messages(self, display_buffer=True, display_messages=False):
        '''
        Analyses information about messages of ipc and prints them to
        formatter.
    ...
    def _display_signal_block(self, signal_p, analyse_fifo):
        '''
        @brief This method analyses and displays signals used in IPC.
        @param[in] self Pointer to the current object
        @param[in] signal_p IPC signal block pointer
        @param[in] analyse_fifo If true, fifo will be analysed.
        '''
    ...
    def analyse_signals(self, analyse_fifo=False):
        '''
        @brief This method analyses all the IPC signals with the help of
            _display_signal_block.
        @param[in] self Pointer to the current object
        @param[in] analyse_fifo If true, fifo will be analysed.
        '''
        # Look up the debug information, unless we already have.
    ...
    def analyse_data(self, display_buffer=True):
        '''
        @brief this method analyses data channels present in ipc. If True
            paremeter is passed, buffer associated with data channel is
            displayed.
        @param[in] self Pointer to the current object
        @param[in] display_buffer = False
        '''
        # buffers are used to store messages, this import buffer analysis for
        # analysing it
        if display_buffer:
    ...
    def _lookup_debuginfo(self):
        ...
        It may need to be performed more than once if working with live chip
