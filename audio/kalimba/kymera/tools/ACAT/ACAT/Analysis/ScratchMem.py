...
class ScratchMem(Analysis.Analysis):
    ...
    def analyse_per_priority(self):
        """
        @brief analyse_per_priority() displays the total scratch memory
            allocations in DM1, DM2 and none and their
            values per each priority level.
            Furthermore, it also displays the total
            number of users per each priority level.
            The function reads and stores the allocations info
            from per_prio_data in the first for loop in order
            to display the total values first. In the second for
            loop it simply displays the stored values
            for the scratch memory (total, DM1, DM2,
            none) for each priority.
        @param[in] self Pointer to the current object
        """
        dm1_total = 0
        dm2_total = 0
