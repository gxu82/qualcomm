...
def cache_decorator(function):
    """
    @brief An analysis function return value can be cached with this decorator.
    @param[in] function
    """
...
class Analysis(object):
    """
    @brief Base class for all analysis plugins.

    class Analysis(Analysis.Analysis):
        def __init__(self, **kwarg):
            # Call the base class constructor.
            # This call will set all the values from the dictionary as
            # attributes.
            # In this manner chipdata, debuginfo and formatter will be set
    ...
    def __init__(self, **kwargs):
        ...
        Note that it is valid for formatter to be None; if we just want to
        analyse data (and not print anything out) it is not necessary to
        create a pointless formatter object.

        Avoid doing lots of work up-front here; if an error occurs, we might
        not be able to handle it gracefully.
    ...
    def run_all(self):
        """
        @brief Called to perform all the useful analysis this module can do.
    ...
    def set_formatter(self, formatter):
        """
        @brief Sets the formatter for an analysis.
        @param[in] self Pointer to the current object
        @param[in] formatter Pointer to the formatter.
        """
        self.formatter = formatter

    def reset_formatter(self):
        """
        @brief Resets the formatter for an analysis.
        @param[in] self Pointer to the current object
        """
    ...
    def to_file(self, file_name, suppress_stdout=False):
        """
        Set the output of the analysis to a file.
        """
    ...
    def parse_linked_list(self, identifier, next_field_name,
        ...
        * override_element_type - by default each list member is interpreted
        ...
        @param[in] self Pointer to the current object
        @param[in] identifier
        @param[in] next_field_name
        @param[in] override_element_type
        """
    ...
    def find_references(self, value):
        ...
        Returns a tuple of each address that contains that value (empty tuple
        if it was not found anywhere).

        Note: it does not search in memory-mapped registers, only in DM1 and
        DM2.
