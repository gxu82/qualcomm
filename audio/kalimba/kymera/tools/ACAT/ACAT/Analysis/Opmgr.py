...
def splitter_str(self):
    ...
    try:
        ...
        metadata_size = None
        metadata_write_index = None
        metadata_read_index_output_0 = None
        metadata_read_index_output_1 = None
        ...
        for channel in self.helper.parse_linked_list(channels, "next"):
            ...
            internal_buff = channel.get_member(
                "internal"
            ).get_member("buffer").value
            if metadata_read_index_output_0 is None:
                metadata_read_index_output_0 = channel.get_member(
                    "internal").get_member("prev_rd_indexes")[0].value
                metadata_read_index_output_1 = channel.get_member(
                    "internal").get_member("prev_rd_indexes")[1].value
            ...
            if internal_buff != 0:
                ...
                if metadata_size is None:
                    metadata = cbuffer_var.get_member("metadata").value
                    metadata = self.helper.chipdata.cast(
                        metadata, "metadata_list"
                    )
                    metadata_size = metadata.get_member('buffer_size').value
                    metadata_write_index = metadata.get_member(
                        'prev_wr_index'
                    ).value
            ...
            for index, output_buffer in enumerate(output_buffers):
                ...
                if output_buff != 0:
                    ...
                    op_str += insert_spaces(str(buff_var))
                    # the output buffers are transform buffers so content
                    # should already be displayed.

        data = metadata_write_index - metadata_read_index_output_0
        if data < 0:
            data += metadata_size
        percent = (float(data) * 100) / metadata_size
        op_str += "Channel 0 : usage %3.2f%% data %d size %d\n" % (
            percent, data, metadata_size
        )

        data = metadata_write_index - metadata_read_index_output_1
        if data < 0:
            data += metadata_size
        percent = (float(data) * 100) / metadata_size
        op_str += "Channel 1 : usage %3.2f%% data %d size %d" % (
            percent, data, metadata_size
        )

        return op_str

    except (ct.DebugInfoNoVariable, AttributeError, TypeError):
        return Operator._desc_str(self)
...
class Opmgr(Analysis.Analysis):
    ...
    def get_operator(self, opid):
        ...
        raise ct.AnalysisError("No operator found with id: " + hex(opid))
    ...
    def get_capability_list(self):
        ...
        if capability_length == 0:
            raise ct.AnalysisError("Invalid capability table")
    ...
    def get_extra_op_data_type(self, function_address, elf_id):
        ...
        if nearest_label is not None:
            ...
            if address_in_dowload_cap:
                ...
                # which will be used to search in the .lst file.
