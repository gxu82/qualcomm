...
class LogController(object):
    """
    @brief Class for controlling the debuglog enable/disable in a safe way.
        With this class the debuglog enable/disable can be encapsulated in a
        "with" statement.
    """
...
def suspend_log_decorator(sleep_time):
    """
    @brief Creates a function decorator which disables and re-enables debug
        logging (see below) and waits for a predefined time before the
        decorated function call.
    @param[in] sleep_time - time to sleep before the decorated function will
        be called.
    """
    def decorator(func):
        """
        @brief Disables and re-enables debug logging for the function call.
...
def is_debuglog_enabled_decorator(func):
    """
    @brief Only calls the decorated function if debuglog is enabled.
    @param[in] func
    """
...
class DebugLog(Analysis.Analysis):
    ...
    def __init__(self, **kwarg):
        ...
        try:
            ...
            # This variable can go out of sync if other tool is modifying the debug
            # log level, but we are only using it to get the address of the debug log
            # level variable so that's fine.
            ...
            # debug log is disabled for the current build. We still need the analysis
            # because the profiler and fats analyses are calling it.
    ...
    def run_all(self):
        """
        @brief Outputs the contents of the debug log buffer.
        @param[in] self Pointer to the current object
        """
    ...
    def get_debug_log(self):
        """
        @brief Gets the (decoded) contents of the debug_log buffer.
        Returns a list of debug strings, ordered from oldest to newest.
    ...
    def set_debug_log_level(self, set_level):
        """
        @brief Sets the debug log level to use.
        ...
        if set_level > 5 or set_level < 0:
            raise ct.Usage("Level must be in the range 0-5\n")
        else:
    ...
    def print_log_level(self):
        """
        @brief Prints the current debug log level.
        ...
        self.log_level = self.chipdata.get_var_strict('$_debugLogLevel')
        self.formatter.alert(
            "Debug log level is currently set to " + str(self.log_level.value)
            + "\n"
        )
    ...
    def poll_debug_log(self, wait_for_keypress=None):
        """
        Polls the debug_log buffer. If the contents of the buffer changes then
        it prints out the new extra contents.

        This function spawns a new thread to perform the polling operation

        This is only available on a live chip.

        This could raise a Usage exception.
        @param[in] self Pointer to the current object
        """
        if wait_for_keypress == None:
            if os.name == 'nt':
    ...
    def stop_polling(self):
        """
        @brief Sets a flag that the polling thread can see telling it to stop
        @param[in] self Pointer to the current object
        """
        self._polling.clear()
    ...
    def logging_active(self):
        """
        @brief Returns True or False depending on whether ACAT is actively
            logging the debuglog.
        @param[in] self Pointer to the current object
        """
        if self.is_debuglog_enabled:
    ...
    def set_formatter(self, formatter):
        """
        @brief Sets how the result is being formatted.
        @param[in] self Pointer to the current object
        @param[in] formatter
        """
    ...
    def analyse_debug_log(self):
        """
        @brief Outputs the contents of the debug_log buffer.
            If something goes wrong (Debug Log not compiled in, Debug Log
            is being polled etc.), an exception is thrown.
        @param[in] self Pointer to the current object
        """
...
class Logging(threading.Thread):
    """
    @brief This class is used to create a separate thread for logging debug
        information
    @param[in] self Pointer to the current object
    """
    ...
    def _get_formatter_string(self, string_ptr, arguments):
        """
        Searches for the formatter string in the debug information. Also checks
        the downloadable capabilities.

        @param[in] self Pointer to the current object
        @param[in] string_ptr Pointer to the formatter string
        @param[in] arguments Arguments for the formatter string. Note this can
                             change.
        """
    ...
    def _format_string(self, formatter, arguments):
        """
        Searches for the formatter string in the debug information. Also
        checks the downloadable capabilities.
        ...
        try:
            ...
            if re.search("arguments number is required", str(exception)):
                ...
                for idx, val in enumerate(specifiers):
                    ...
                    if not re.search("[cs]", val) and isinstance(
                            arguments[idx], str
                        ):
    ...
    def decode_log(self):
        """
        @brief Decodes the contents of the debug_buffer and returns it.

        This could raise a number of different exception types, including
        AnalysisError.

        This function could be called from outside of this thread. A lock is
        used to makes sure it is only running on one thread.
    ...
    def _decode_log(self):
        """
        @brief Same as decode_log, but is not thread safe.
        @param[in] self Pointer to the current object
        """
                ...
                if mystr.count('%') > len(arguments):
                    ...
                    if num_args_missing == mystr.count('%s'):
                        ...
                        if Arch.addr_per_word != 4:
                            # For Hydra platforms, like Crescendo, Stre and
    ...
    def run(self):
        """
        Polls the debug log buffer repeatedly. This is called in a separate
        thread so that command line input still works
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                if isinstance(new_debug_log, list):
                    ...
                    TypeError, IndexError, ct.AnalysisError
                ) as excep:  # possible if the buffer tears
